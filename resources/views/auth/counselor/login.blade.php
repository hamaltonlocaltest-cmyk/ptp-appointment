...
<head>
	...
	<style>
		.login-page { /*background: #1b5e20;*/ background: url("{{ asset('images/bg-counselor-login.webp') }}") no-repeat center center; background-size:cover }
		...
		.login-card{width:100%;  max-width:470px; background:rgba(255,255,255,.95); border-radius:28px; padding:40px; box-shadow:0 30px 60px rgba(0,0,0,.18); position:relative; z-index:10;}
		...
		.form-control{height:52px; border-left:none; border-right:none;}
...
<div class="login-wrapper">
    ...
    <div class="login-card">
        ...
        <p class="subtitle">
            Sign in to your counselor account
        </p>

		 @if ($errors->any())
                <div class="alert alert-modern-danger py-2">{{ $errors->first() }}</div>
            @endif

            @if (session('message'))
                <div class="alert alert-modern-success py-2">{{ session('message') }}</div>
            @endif

        <form action="{{ route('counselor.login') }}" method="POST">
			 @csrf
        ...
        <a href="{{ route('counselor.register') }}" class="create-account">
...
<div class="login-box d-none">
    ...
    <div class="card">
        <div class="card-body login-card-body">
            <div class="role-badge"><span><i class="fas fa-user-md"></i> Counselor</span></div>
            <p class="login-box-msg text-muted">Sign in to your counselor account</p>
            ...
            <p class="mb-1 mt-3 text-center">
                <a href="{{ route('counselor.register') }}" style="color:#1b5e20;">Create a counselor account</a>
            </p>
