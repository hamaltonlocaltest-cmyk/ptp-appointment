...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('counselor.complaints.index') }}">Complaints</a></li>
...
<div class="col-lg-7">
    ...
    <div class="mb-4">
        ...
        <p class="text-muted mb-0" style="font-size:13px;">
            Report an issue with a counselee or a session. Our admin team will review it.
        </p>
    </div>
    ...
    <div class="card" style="border-radius:14px; border:none; box-shadow:0 4px 20px rgba(0,0,0,.08);">
        <div class="card-body p-4">
            <form action="{{ route('counselor.complaints.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="appointment_id" class="form-control" style="border-radius:8px; border:1.5px solid #e0e4ec; font-size:13px; padding:8px 12px;">
                        ...
                        <option value="{{ $appt->id }}" {{ old('appointment_id') == $appt->id ? 'selected' : '' }}>
                            {{ $appt->counselee->full_name }} — {{ $appt->counselType->name }} — {{ $appt->appointment_date->format('M j, Y') }}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mt-4 d-flex justify-content-between">
                    <a href="{{ route('counselor.complaints.index') }}" class="btn btn-secondary"
                       style="border-radius:7px; padding:10px 22px; font-weight:700;">
                        <i class="fas fa-arrow-left mr-1"></i> Cancel
                    </a>
                    <button type="submit" class="btn btn-primary"
                            style="color:#fff; border-radius:7px; padding:10px 30px; font-weight:700; ">
